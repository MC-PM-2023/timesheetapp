...
# # 🆕 Import the db object from your new models file
# from models import db, User, ProcessTable, ProjectCode, UserProjectAssignment, UserProfile
# ── CONFIG ────────────────────────────────────────────────────────────────
app = Flask(__name__)
...
def start_timer():
    ...
    # Store timer details in session
...
def stop_timer():
    ...
    team = User.query.filter_by(username=name).first().team  # Get user's team
    
    start_dt = datetime.fromisoformat(timer_data["start_time"])
    end_dt = datetime.now()
    
    if end_dt <= start_dt: # Prevent negative duration
    ...
    # Parse project fields
    proj_code, proj_type_mc, disease, country = parse_project_fields(team, timer_data["project"])
    ...
    cur.execute("""
        ...
        VALUES
          (%s, %s, %s, %s, %s, %s, %s, %s,
           %s, %s, %s, %s,
           %s, %s, %s, %s)
    """, (name, date_str, end_dt.strftime("%A"), team, 
          timer_data["project"], "WIP", # Assuming WIP for timer
          timer_data["process"], timer_data["sub_process"],
          start_dt.strftime("%H:%M"), end_dt.strftime("%H:%M"), 
          duration_str, total_h,
          proj_code, proj_type_mc, disease, country))
    mysql.connection.commit()
    cur.close()
